...
int main() {
    ...
    struct Contato contato; // contato é uma variável do tipo struct Contato
    char nomeArquivo[50];
    ...
    while (1) { //true
        ...
        fprintf(arquivo, "%-*s %-*s\n", 30, contato.nome, 15, contato.telefone);
    }

    // Fecha o arquivo
    fclose(arquivo);

    printf("Cadastro concluido. O arquivo %s foi criado.\n", nomeArquivo);
